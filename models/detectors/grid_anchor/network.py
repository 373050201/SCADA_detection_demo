...
class GridAnchorDetector(nn.Module):
    #输入shape:[B, 3, input_size, input_size]
    #输出shape:[B, grid_size, grid_size, 5, (5+nc)],其中(5+nc)包括(tx,ty,tw,th,conf,nc个onehot)
    def __init__(self):
        ...
        self.grid_size=self.input_size//32#一行的网格个数
        #1、骨干网络(用于提取特征)
        self.backbone=nn.Sequential(*list(resnet.children())[:-2])#除去resnet最后两层
        #shape:[B,512,grid_size,grid_size]
        ...
        self.enhance=nn.Sequential(
            ...
            nn.BatchNorm2d(1024),
            nn.LeakyReLU(),
            #shape:[B,1024,grid_size,grid_size]
        ...
        self.process=nn.Sequential(
            ...
            #shape:[B,5*(5+nc),grid_size,grid_size]
        )
    ...
    def forward(self,x):
        ...
        x=self.enhance(x)
        x=self.process(x)
        x=x.permute(0,2,3,1)#重排为[B,grid_size,grid_size,5*(5+nc)]
        B,grid_y_size,grid_x_size,_=x.shape
        x=x.view(B,grid_y_size,grid_x_size,5,5+self.nc)#重塑为[B,grid_size,grid_size,5,(5+nc)]
